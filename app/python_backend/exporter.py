...
def run_export(
    ...
    # 2026-09-03: B-roll frame-rate interpretation duplication now happens
    # inside precut_pipeline/multi_exporter.py's own B-roll master-clip
    # loop (a structural change to the bin-building loop itself, not
    # something that can be bolted on as post-processing the way the
    # earlier reference-sequence attempt was) -- see that file's own
    # comment at the loop, and posthouse/broll_interpret.py's module
    # docstring for the two prior approaches this superseded, both
    # falsified in real Premiere testing.
