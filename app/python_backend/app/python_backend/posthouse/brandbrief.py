...
def build_brand_section(
    ...
    *,
    ...
    # Skip macOS `._*` AppleDouble sidecars and other dotfiles -- metadata,
    # never a real asset. Same convention as projectmanager._is_hidden
    # (not imported directly: that name is private to that module), found
    # necessary the same way there -- a real run on a network/external
    # volume (RDOSS_2025) hit `._SF-Main-RE-light.png`, which carries the
    # real file's extension and so matched LOGO_EXTS, then failed to
    # decode as an image with UnidentifiedImageError.
    all_files = sorted(
